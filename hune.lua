    ...
    for _,obj in pairs(workspace:GetDescendants()) do
        if obj:IsA("Model") and obj:FindFirstChild("Handle") then
            ...
            for _,serverName in ipairs(FRUITS) do
                if string.find(string.lower(obj.Name), string.lower(serverName), 1, true) then
                    matched = true break
                end
            end
            if matched or string.find(string.lower(obj.Name), "fruit") then
...
local function storeAllFruitOnce()
    ...
    for _,obj in pairs((backpack and backpack:GetChildren()) or {}) do
    ...
    for _,obj in pairs((char and char:GetChildren()) or {}) do
        if string.find(obj.Name, "Fruit") then
            pcall(function()
                ReplicatedStorage.Remotes.CommF_:InvokeServer("StoreFruit", obj.Name, obj)
            end)
    ...
    while task.wait(0.5) do
        ...
        if #fruitsFound > 0 then
            for _,fruitModel in ipairs(fruitsFound) do
                ...
                pcall(function()
                    StarterGui:SetCore("SendNotification", {Title = "HuneIPA - Fruit Finder", Text = "Find {" .. tostring(totalFruit) .. "} fruit on this server", Duration = 5})
                end)
                local remain = findFruitsInWorkspace()
                if #remain > 0 then
